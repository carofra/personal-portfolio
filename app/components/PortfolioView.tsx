...
function ProjectSlideCard({ project, index }: SectionProps) {
  ...
  const ui = styleByIndex[index % styleByIndex.length];
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="relative z-0 mt-2 max-w-xl text-base leading-snug text-white/95 sm:text-lg lg:text-xl">
          {project.description}
        </p>
        ...
        <div className="mt-8 grid grid-cols-1 border border-white/15 sm:mt-9 sm:grid-cols-2 lg:mt-10">
...
function ProjectsStepsSection({ projects }: { projects: ProjectEntry[] }) {
  return (
    <section className="w-full border-t border-zinc-200/80 px-4 pt-4 pb-14 sm:px-8 sm:pt-6 sm:pb-18 lg:px-16 lg:pt-8 lg:pb-28">
      <div className="mx-auto w-full max-w-[92rem]">
        <ul role="list" className="space-y-8 sm:space-y-12 lg:space-y-20">
          {projects.map((project, index) => (
            <ProjectSlideCard key={project.title} project={project} index={index} />
          ))}
        </ul>
      </div>
    </section>
...
function PortfolioInner({
  ...
  const heroX = useTransform(scrollY, [0, 220], [0, -560]);
  const heroY = useTransform(scrollY, [0, 220], [0, -320]);
  const heroScale = useTransform(scrollY, [0, 220], [1, 0.24]);
  const heroOpacity = useTransform(scrollY, [0, 220, 320], [1, 1, 0]);
  const hintOpacity = useTransform(scrollY, [0, 180, 240], [1, 1, 0]);
  ...
  return (
    <main className="relative w-full overflow-x-clip bg-[#FAFAFA] text-[#111]">
      ...
      <ProjectsStepsSection projects={projectList} />
